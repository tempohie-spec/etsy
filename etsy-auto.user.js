...
(function () {
  ...
  function moBangChonAnh(goi, soAnhDangCo, conLai, soBangSizeCoDinh = 0) {
    return new Promise((resolve) => {
          ...
          ${
            soAnhDangCo > 0
              ? `<div style="margin-bottom:6px;padding:8px;background:#FFFBEB;border:1px solid #FDE68A;border-radius:6px;line-height:1.5;">
                 ...
                 <label style="display:block;cursor:pointer;"><input type="radio" name="ea-up-xong" value="khong" checked disabled autocomplete="off"> Dừng lại sau khi upload, tôi tự kéo ảnh + tự lưu</label>`
              : `<div style="margin-bottom:6px;font-weight:bold;">Sau khi upload xong:</div>
                 <label style="display:block;margin-bottom:3px;cursor:pointer;"><input type="radio" name="ea-up-xong" value="publish" autocomplete="off"> Bấm hộ <b>Publish copy with changes</b> — đăng bán công khai ngay, khó lùi lại</label>
                 <label style="display:block;cursor:pointer;"><input type="radio" name="ea-up-xong" value="khong" checked autocomplete="off"> Dừng lại, tôi tự bấm lưu</label>`
          }
        </div>
      ...
      // Etsy (trang SPA, khong reload trang thuc su) cung voi tinh nang "nho lai gia tri form" cua
      // trinh duyet co the giu lai radio da chon o LAN TRUOC (cung "name") va tu tich san no vao
      // lan mo hop thoai KE TIEP, du HTML vua nhoi vao luon ghi "khong" la checked. Ep lai bang tay
      // (gan .checked THAY VI chi dua vao thuoc tinh checked trong chuoi HTML) de dam bao luon dung
      // mac dinh "Dừng lại, tôi tự bấm lưu" (hoac "khong" disabled khi listing dich da co san anh).
      const oRadioMacDinh = hop.querySelector('input[name="ea-up-xong"][value="khong"]');
      if (oRadioMacDinh) oRadioMacDinh.checked = true;
  ...
  //
  // v9.3 tung giam xuong 2 vi nghi bi chan ngam do goi qua nhieu/qua nhanh, sau khi gap hang loat
  // request treo hoan toan khong loi khong phan hoi. Nguoi dung sau do tu kiem chung: dan THANG
  // URL dang loi vao 1 tab thuong (khong qua script) thi anh TAI DUOC BINH THUONG — chung to
  // khong phai bi CDN/tien ich chan, ma chi la mang cua nguoi dung cham/chap chon dung luc test.
  // Gia thuyet "chan ngam do goi dong dap" bi loai bo, nen tra lai 4 (giam xuong 2 chi lam moi
  // lan upload cham hon ~60-70% ma khong giai quyet duoc van de gi thuc su ton tai).
  ...
  // ================== VARIATIONS: COPY / DAN / CHI DIEN GIA ==================
  // Gop tu script rieng "Etsy Variations Copy/Paste" (v1.3). Boc trong 1 IIFE rieng de cac ham tro
  // giup cung ten (dangHienThi, setNativeValue, bam...) KHONG de len ham cua phan con lai.
  //   - Copy variations : doc cac bang Variations (ten variation, ten option, gia, Visible) o listing
  //                        nguon, ghi vao Clipboard he thong (dan duoc sang trinh duyet khac) + GM_setValue
  //   - Dan variations  : Add variation -> Create your own -> Name + tung option (Add) -> Done,
  //                        lap lai qua "Add a variation"; bat Prices vary + chon variation co gia -> Apply;
  //                        roi dien gia va bat/tat Visible
  //   - Chi dien gia    : variations da co san -> chi dien gia + Visible, khop theo ten option
  const BienThe = (function () {
    const log = (...a) => console.log('[Etsy Auto][Variations]', ...a);
    const canhBao = (...a) => console.warn('[Etsy Auto][Variations]', ...a);
    ...
    function vungHopThoai() {
      ...
      ].filter((el) => dangHienThi(el) && !el.closest('#etsy-auto-panel'));
    ...
    function cacNutHienThi(goc) {
      return [...goc.querySelectorAll('button, [role="button"], [role="menuitem"], [role="option"]')].filter(
        (b) => dangHienThi(b) && !b.disabled && b.getAttribute('aria-disabled') !== 'true' && !b.closest('#etsy-auto-panel')
      );
    }
  ...
  function taoGiaoDien() {
    ...
    // Nhom nut Variations chi co y nghia tren trang tao/sua listing
    document.getElementById('ea-bienthe-khung').style.display = location.pathname.includes('/listing-editor')
      ? 'flex'
      : 'none';
    document.getElementById('ea-btn-var-copy').onclick = BienThe.copy;
    document.getElementById('ea-btn-var-paste').onclick = BienThe.dan;
    document.getElementById('ea-btn-var-price').onclick = BienThe.chiDienGia;
